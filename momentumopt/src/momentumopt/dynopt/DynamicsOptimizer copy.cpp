...
namespace momentumopt {
  ...
  void DynamicsOptimizer::internalOptimize(const KinematicsSequence& kin_sequence, bool is_first_time)
  {
    try
    {
      ...
      std::fstream file1;
      file1.open("/home/jhk/walkingdata1/stairdown/25cm/ssp2/timestep=88/timestep0_zmp3_ssp1_1.txt",std::ios_base::out);    
      ...
      std::fstream file;
      file.open("/home/jhk/walkingdata1/stairdown/25cm/ssp2/timestep=88/timestep0_zmp2_ssp1_1.txt",std::ios_base::out);
      ...
      for (int time_id=0; time_id<this->getSetting().get(PlannerIntParam_NumTimesteps); time_id++) {
        ...
        if(time_id >= 0)
        {
          ...
          if(zmp_double == true)
          {
            quad_objective_.addQuaTerm(this->getSetting().get(PlannerVectorParam_WeightZMPC)[0]/100, (LinExpr(vars_[ZMP_.id(0,time_id)]) - LinExpr(zmpx)));
            quad_objective_.addQuaTerm(this->getSetting().get(PlannerVectorParam_WeightZMPC)[1]/100, (LinExpr(vars_[ZMP_.id(1,time_id)]) - LinExpr(zmpy)));
          }
        ...
        if(time_id == 0)
        {
          double  comddz, comz, zmpz, comy, zmpy, angx;
          comddz = dynamicsSequence().dynamicsState(time_id).linearMomentumRate()[2]/this->getSetting().get(PlannerDoubleParam_RobotMass);
        ...
        else
        {
          ...
          zmpy = dynamicsSequence().dynamicsState(time_id).ZMP()[1];
          angx = dynamicsSequence().dynamicsState(time_id).angularMomentumRate()[0];
          lin_cons_ += (LinExpr(vars_[lmomd_.id(1,time_id)])/this->getSetting().get(PlannerDoubleParam_RobotMass) - LinExpr(vars_[lmomd_.id(1,time_id-1)])/this->getSetting().get(PlannerDoubleParam_RobotMass)) - (9.81 + comddz)/(comz-zmpz) * (LinExpr(vars_[com_.id(1,time_id)]) - LinExpr(vars_[com_.id(1,time_id-1)]) - (LinExpr(vars_[ZMP_.id(1,time_id)]) - LinExpr(vars_[ZMP_.id(1,time_id-1)]))) + (LinExpr(vars_[amomd_.id(0,time_id)])-LinExpr(vars_[amomd_.id(0,time_id-1)]))/(this->getSetting().get(PlannerDoubleParam_RobotMass)*(comz-zmpz));
          lin_cons_ += 1/(comz-zmpz)*(comy-zmpy+angx/(this->getSetting().get(PlannerDoubleParam_RobotMass) * (9.81+comddz))) *(LinExpr(vars_[lmomd_.id(2,time_id)])/this->getSetting().get(PlannerDoubleParam_RobotMass) - LinExpr(vars_[lmomd_.id(2,time_id-1)])/this->getSetting().get(PlannerDoubleParam_RobotMass));
          lin_cons_ += -(9.81 + comddz)/((comz-zmpz)*(comz-zmpz))*(comy-zmpy+angx/(this->getSetting().get(PlannerDoubleParam_RobotMass) * (9.81+comddz)))*(LinExpr(vars_[com_.id(2,time_id)]) - LinExpr(vars_[com_.id(2,time_id-1)]) - (LinExpr(vars_[ZMP_.id(2,time_id)]) - LinExpr(vars_[ZMP_.id(2,time_id-1)])));
          model_.addLinConstr(lin_cons_, "=", 0.0);
        }
